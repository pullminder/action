...
## CI mode with coverage webhook

When `mode: ci` is set, the action runs `pullminder ci --json --github-annotations` against the repository and can optionally upload test coverage data directly to Pullminder's webhook endpoint — no GitHub artifact storage needed.

### Coverage webhook mode

Coverage webhook mode sends test coverage data from your CI pipeline directly to Pullminder via HTTP. This replaces the old GitHub Actions artifact workflow (`workflow_run` → download artifact → parse) with a single direct POST, eliminating artifact storage costs.

**Prerequisites:**
- A Pullminder organization on the Team+ plan
- Coverage webhook enabled on the repository (generates a `pm_cov_` token via [dashboard settings](https://app.pullminder.com/settings))
- `jq` available on the runner (included on all GitHub-hosted runners)
- Coverage output in a supported format (lcov, Go cover, Cobertura XML)

### Full workflow: run tests and upload coverage

```yaml
name: Pullminder CI with coverage
on: pull_request
...
jobs:
  ci:
    ...
    steps:
      - uses: actions/checkout@v4
      - uses: pullminder/action@v2
        with:
          mode: ci
          comment: "true"
          fail-on: high
          coverage-mode: webhook
          coverage-webhook-token: ${{ secrets.PULLMINDER_COVERAGE_TOKEN }}
          coverage-test-command: "npm test -- --coverage"
```

### Upload coverage only (tests run in a separate step)
...
For security, store the token in an environment variable rather than an action input:

```yaml
- uses: pullminder/action@v2
  env:
    PULLMINDER_COVERAGE_TOKEN: ${{ secrets.PULLMINDER_COVERAGE_TOKEN }}
  with:
    coverage-mode: webhook
```

When both the `coverage-webhook-token` input and `PULLMINDER_COVERAGE_TOKEN` env var are set, the input takes precedence.

### Auto-detected coverage file paths

If `coverage-file` is not specified, the action searches in this order:

1. `lcov.info`
2. `cover.out`
3. `coverage/lcov.info`
4. `coverage/coverage.info`
5. `coverage/lcov.dat`
6. `coverage.out`

Use `coverage-file` to specify a custom path when your coverage output goes elsewhere.

### Coverage webhook inputs

| Input                      | Default                                                          | Description                                                                                     |
| -------------------------- | ---------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- |
| `coverage-mode`            | `artifact`                                                       | Set to `webhook` to POST coverage directly to Pullminder.                                       |
| `coverage-webhook-token`   | (empty)                                                          | Bearer token for webhook auth (use secrets!). Prefer `PULLMINDER_COVERAGE_TOKEN` env var.       |
| `coverage-webhook-url`     | `https://api.pullminder.com/api/v1/coverage/webhook`             | Override the webhook endpoint URL (e.g., for staging).                                          |
| `coverage-file`            | (empty)                                                          | Explicit path to the coverage file. Skips auto-detection when set.                              |
| `coverage-test-command`    | (empty)                                                          | Shell command to run before uploading (e.g., `go test -coverprofile=cover.out ./...`).          |

### File size limit

The webhook endpoint accepts payloads up to 10 MB. The action checks the coverage file size before encoding and fails early if the raw file exceeds 7 MB (approximately 10 MB after base64 encoding). If your coverage file is too large, consider:

- Reducing coverage scope to changed files only
- Splitting reports across multiple jobs
- Using a coverage tool that produces more compact output
